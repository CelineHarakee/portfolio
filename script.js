...
if (typingCodeElement) {
    ...
    function formatCode(text) {
        return text
            .replace(/developer/g, '<span class="text-green-600">developer</span>')
            .replace(/languages/g, '<span class="text-black-600">languages</span>')
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const wrapper = document.getElementById('ecosystem-wrapper');
    const container = document.getElementById('graph-container');
    const graphArea = document.getElementById('graph-area');

    const sectionProjects = document.getElementById('section-projects');
    const sectionPapers = document.getElementById('section-papers');
    const gridProjects = document.getElementById('grid-projects');
    const gridPapers = document.getElementById('grid-papers');
    ...
    function createNodes() {
        networkData.forEach(item => {
            ...
            // DYNAMIC BADGE LOGIC INJECTED HERE (Removed margins that forced it inline poorly)
            ...
            if (item.type === 'project') gridProjects.appendChild(node);
            else gridPapers.appendChild(node);
        });
    }
    ...
    function updateHighlights() {
        ...
        if (hasHover) graphArea.classList.add('has-hover');
        else graphArea.classList.remove('has-hover');
        ...
        if (hoveredNodeId && currentFilter === 'all') {
            ...
            nodeElements[hoveredNodeId].classList.add('highlighted');

            networkData.forEach(item => {
                if (item.id !== hoveredNodeId && isVisible(item)) {
                    const shared = getSharedTags(hoveredItem, item);
                    if (shared.length > 0) nodeElements[item.id].classList.add('highlighted');
                }
            });
        }
    }
    ...
    function updateDetailsPanel(item) {
        ...
        document.getElementById('link-text').textContent = item.type === 'project' ? 'View Repository' : 'Read Paper';
        document.getElementById('link-icon').className = item.type === 'project' ? 'fa-brands fa-github' : 'fa-solid fa-arrow-right';
    }
        ...
        btn.addEventListener('click', (e) => {
            ...
            if (activeNodeId) {
                ...
                if (!isVisible(activeItem)) {
                    activeNodeId = null;
                    document.getElementById('details-empty').style.opacity = '1';
                    document.getElementById('details-empty').style.pointerEvents = 'auto';
                    document.getElementById('details-content').style.opacity = '0';
                    document.getElementById('details-content').style.pointerEvents = 'none';
                }
            }

            if (currentFilter === 'all') {
                graphArea.className = 'w-full flex justify-between z-10 relative';
                sectionProjects.style.display = 'flex';
                sectionPapers.style.display = 'flex';
                sectionProjects.className = 'w-1/2 flex flex-col items-start relative z-10';
                sectionPapers.className = 'w-1/2 flex flex-col items-end relative z-10';
                gridProjects.className = 'flex flex-col gap-4 w-full';
                gridPapers.className = 'flex flex-col gap-4 w-full items-end';
                canvas.style.display = 'block';
            } else if (currentFilter === 'project') {
                graphArea.className = 'w-full block z-10 relative';
                sectionPapers.style.display = 'none';
                sectionProjects.style.display = 'block';
                sectionProjects.className = 'w-full relative z-10';
                gridProjects.className = 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 w-full';
                ...
                gridPapers.className = 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 w-full';
                canvas.style.display = 'none';
            }
